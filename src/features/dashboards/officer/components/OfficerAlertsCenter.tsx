  ...
  return (
    <div className="min-h-[380px] xl:min-h-[415px] max-h-[640px] xl:max-h-none xl:h-full xl:relative">
      <aside
        ...
        className="flex flex-col min-w-0 w-full h-full xl:absolute xl:inset-0 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"
      >
        <div className="flex shrink-0 items-center justify-between border-b border-slate-200 bg-slate-50 px-5 py-4">
          <div className="flex items-center gap-3 min-w-0">
            <Bell
              aria-hidden="true"
              className="h-5 w-5 shrink-0 text-[#0A3C2F]"
            />
            <h2
              id="alerts-center-title"
              className="text-lg font-semibold text-[#16253d] truncate"
            >
              Alerts Center
            </h2>
          </div>
        ...
        <div className="flex-1 p-4 overflow-y-auto">
          {loading ? (
            ...
            <div className="flex flex-1 h-full min-h-[220px] flex-col items-center justify-center py-6 text-center text-sm text-slate-500">
              <CheckCircle2 className="h-7 w-7 text-[#0A3C2F]/40 mb-1.5" />
              ...
              {alerts.map((alert) => {
                ...
                return (
                  <Link
                    key={alert.id}
                    href={alert.href}
                    className="group flex items-stretch gap-3 sm:gap-3.5 rounded-md border border-slate-200/90 bg-white p-3 sm:p-3.5 shadow-2xs transition-all hover:border-[#0A3C2F]/40 hover:bg-slate-50/40 hover:shadow-xs focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#0A3C2F]"
                  >
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center gap-0.5 sm:gap-1">
                      ...
                      <div
                        className={`text-xs sm:text-sm font-semibold tracking-tight break-words [overflow-wrap:anywhere] [word-break:break-word] leading-snug group-hover:underline ${
                          alert.tone === "returned"
                            ? "text-[#10243f]"
                            : "font-mono text-[#0A3C2F]"
                        }`}
                      >
                        {alert.referenceLine}
                      </div>
                      ...
                      {alert.tone === "delayed" && (
                        <div className="mt-2 space-y-1.5">
                          ...
                          <button
                            ...
                            className="px-2.5 py-1 rounded-md text-[11px] font-semibold bg-white text-rose-700 border border-rose-200 hover:bg-rose-50 flex items-center gap-1.5 transition-colors cursor-pointer shadow-2xs w-fit"
                      ...
                      <div className="mt-2 pt-2 border-t border-slate-100 flex items-center justify-between">
                        ...
                        <span className="inline-flex items-center gap-1 text-xs sm:text-sm font-semibold text-[#0A3C2F] group-hover:text-[#006837] group-hover:underline">
